...
from .models import VisitSlot, Visit, DirectBookingInquiry
...
class VisitSlotAdmin(admin.ModelAdmin):
    ...
    list_filter = ['tour_type', 'is_active', 'start_at', 'agent']
    search_fields = ['listing__title', 'agent__username', 'meeting_location', 'notes']
    readonly_fields = ['id', 'available_capacity', 'is_full', 'is_past', 'created_at', 'updated_at']
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('listing', 'agent').prefetch_related('visits')
...
class VisitAdmin(admin.ModelAdmin):
    ...
    list_filter = ['status', 'selected_tour_type', 'booking_intent', 'fee_paid', 'created_at']
    search_fields = [
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('listing', 'buyer', 'slot', 'slot__agent')
...
class DirectBookingInquiryAdmin(admin.ModelAdmin):
    ...
    search_fields = ['visit__listing__title', 'visit__buyer__username', 'buyer_message', 'agent_response']
    readonly_fields = ['id', 'is_expired', 'created_at', 'updated_at', 'responded_at']
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('visit', 'visit__buyer', 'visit__listing')

    actions = ['mark_responded', 'mark_accepted']

    def mark_responded(self, request, queryset):
        updated = queryset.filter(status='pending').update(status='responded', responded_at=timezone.now())
